...
const filters = {
  default: () => true,
  'filter-needs-work': (pullRequest) =>
    pullRequest.reviewers.some(
      (reviewer) =>
        reviewer.status === 'NEEDS_WORK' ||
        pullRequest.properties.openTaskCount > 0,
    ),
  'filter-no-reviewer': (pullRequest) =>
    !pullRequest.reviewers || !pullRequest.reviewers.length,
};
...
const fetchAll = async (config) => {
  ...
  const urlWithoutReqParams = `${baseUrl}/rest/api/1.0/projects/${repoName.toUpperCase()}/repos/${projectName.toLowerCase()}/pull-requests`;
  ...
  while (!isLastPage && pullRequests.length < numberOfItems) {
    const url =
      urlWithoutReqParams + (nextStartPage ? `?start=${nextStartPage}` : '');

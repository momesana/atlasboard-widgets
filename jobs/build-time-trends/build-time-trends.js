  ...
  return {
    ...
    data: response.data.builds
      .filter(
        ({ description }) =>
          description &&
          (!Object.keys(descriptionFilters).includes(job) ||
            description.includes(descriptionFilters[job])),
      )
      .map(({ duration, displayName, timestamp }) => ({
        duration,
        displayName,
        timestamp,
      }))
...
module.exports = {
  onRun: async (config, _dependencies, jobCallback) => {
    try {
      ...
      const { numberOfItems, baseUrl, jobs, descriptionFilters, widgetTitle } =
        config;

      const fetchForJob = (job) =>
        fetchData(
          baseUrl,
          username,
          password,
